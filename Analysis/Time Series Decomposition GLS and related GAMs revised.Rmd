...
needs.  We fit three classes of models.
...
terms (and fit with $gls()$) or fit using a cyclic cubic spline function through
$gamm()$.
...
Sevreral highly regarded packages, including $zoo$ and $xts$ provide tools to address
...
First, we creat a "complete" data set, with NAs added as necessary to fill all
...
```
Oh oh....  Some timestamps are on the half hour.  All are from late in 2018.
...
be built with ML, not REML methods. Once an optimal model is selected, it can be
refit with REML.
...
We can now look at autocorrelation and crosscorrelaiton structure. However,
these analysis don'thandle seasonal patterns or winter data gaps correctly.
...
Many of hte models I explored took too much memory and either failed to run, or
...
of searching online failed to find a ready way to run gls or gam using tools that
...
because of how it handles co-variation of dependent and independnet variables at
different temporal scales.

Finally, I have learned to save the results of these long-running calculation so I
can reload them rather than recaclulate them.  R Markdown includes an option to
...
allows the shape t odeviate somewhat from a simple sinusidal curve.  One could
...
By calling corAR1() with no arguments, or with a form argument of 'frm = 1',
...
at any data gaps so pairs of observations on either side of  gap in data will
not be included in estimation.

Fitting a more correct model by using a form argument, such as 'form = ~
datetime' tends to use even more memory.  Fitting a model with a grouping
factor, such as 'form = ~ datetime | yyyy' or 'form = ~ 1 | yyyy' reduced memory
demands.
...
# Informal plot of the time of day factor.
```{r}
...
That suggests a four term sinusoidal fit is inadequate o that we have wide
(unreported here) error bands around the sinusoidal fit.
```{r}
...
This took a bit over half an hour on my home computer without the grouping variable, and again failed on my work computer.  With the grouping variable, it takes a bit less than five minutes. 
```{r}
...
```
They do.  Some periodic structure remains with a roughly 12 or 24 hour period. That is an indication that we have not yet captured all the structure in this time series. We should interpret model results wit ha certain amount of caution.
...
Thos model took just over five minutes to run.
```{r}
rm(the_gam_mm)
gc()
...
Those terms look remarkably similar to what we saw in the last model. Looking t teh model sumamries shows the similarities extend to estimates og the linear model terms. Functionally, the two models give similar results.
```{r}
...
The magnitude of the seasonal pattern has been reduced slightly, as some of the structure has been absorbed into the joint smooth.  The joint smoothing term (which tests as significantly significant if you look at the model summaries)  shows that the base diurnal patterns is substantially weakened during winter, and strengthened in late summer and fall.  However, we have relatively little winter data, so the strength of this pattern in winter is probably suspect.
